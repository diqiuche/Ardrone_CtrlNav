...
int32_t vp_api_get_thread_idx_tab_by_name(thread_table_entry_t* tab, const char* name)//start_thread先进入
{
  int32_t i = 0;
...
C_RESULT vp_api_start_thread_tab(thread_table_entry_t* tab,int32_t idx,THREAD_PARAMS parameters)//后进入
{
  if(idx < 0)
    return C_FAIL;
